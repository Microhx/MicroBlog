...
public class ChangeModeController {
     ...
     * app全局背景
     */
    private static final String ATTR_APP_BACKGROUND = "appBackgroundAttr";
    /**
     * 属性背景
     */
    ...
    private static Set<AttrEntity<View>> mAppBackgroundViews;
    private static Set<AttrEntity<View>> mBackGroundViews;
    private static Set<AttrEntity<View>> mBackGroundDrawableViews;
    private static Set<AttrEntity<TextView>> mOneTextColorViews;
    private static Set<AttrEntity<TextView>> mTwoTextColorViews;
    private static Set<AttrEntity<TextView>> mThreeTextColorViews;
    ...
    private void init() {
        if (mAppBackgroundViews == null ||
            ...
            mAppBackgroundViews = new HashSet<>();
            mBackGroundViews = new HashSet<>();
            mOneTextColorViews = new HashSet<>();
            mTwoTextColorViews = new HashSet<>();
            mThreeTextColorViews = new HashSet<>();
            mBackGroundDrawableViews = new HashSet<>();
        }
                ...
                try {
                    ...
                    if (view != null) {
                        ...
                        for (int i = 0; i < attrs.getAttributeCount(); i++) {
                            ...
                            if (attrs.getAttributeName(i).equals(ATTR_APP_BACKGROUND)) {
                                mAppBackgroundViews.add(new AttrEntity<View>(view, getAttr(mClass, attrs.getAttributeValue(i))));
                            }
    ...
    public static void setTheme(Context ctx, int Style_Day, int Style_Night) {
        if (ChangeModeHelper.getChangeMode() == ChangeModeHelper.MODE_DAY) {
            ctx.setTheme(Style_Day);
        } else if (ChangeModeHelper.getChangeMode() == ChangeModeHelper.MODE_NIGHT) {
    ...
    public static void toggleThemeSetting(Stack<Activity> stack) {
        if (ChangeModeHelper.getChangeMode() == ChangeModeHelper.MODE_DAY) {
            for (Activity axt : stack) {
                changeNight(axt, R.style.NightTheme);
            }
        } else if (ChangeModeHelper.getChangeMode() == ChangeModeHelper.MODE_NIGHT) {
            for (Activity axt : stack) {
                changeDay(axt, R.style.DayTheme);
            }
        }
    }
    ...
    public static void changeNight(Activity ctx, int style) {
        ...
        ChangeModeHelper.setChangeMode(ChangeModeHelper.MODE_NIGHT);
        ctx.setTheme(style);
        showAnimation(ctx);
        refreshUI(ctx);
    }
    ...
    public static void changeDay(Activity ctx, int style) {
        ...
        ChangeModeHelper.setChangeMode(ChangeModeHelper.MODE_DAY);
        ctx.setTheme(style);
        showAnimation(ctx);
        refreshUI(ctx);
    }
    ...
    private static void refreshUI(Activity ctx) {
        ...
        for (AttrEntity<View> entity : mAppBackgroundViews) {
